...
((_)=> {
    ...
    _.Scene.prototype.exFun=async function(eve){
          switch (eve[0]) {
            ...
            case "#setStory":
                if(this._item["t_text"]) {
                    this._item["t_text"]._txt = []
                    for (let item of eve[1])
                        this._item["t_text"]._txt.push(this._word[item].text)
                }
               break
            case "#setFormer":
                if(this._item["t_text"]&&this._story._per[eve[1]])
                   this._item["t_text"]._per=this._story._per[eve[1]]
                break
            case "#size":
                  if(this._item[eve[1]]){
            ...
            case "#pushData":
                 if(this._item[eve[1]]){
                     if(this._item[eve[1]] instanceof LIM.SCENE.Shape){
                         this._item[eve[1]]._com.data[eve[2]*1]=LIM.ENTITY.ShapeData()
                     }
                 }
                 break
            case "#setDatas":
                if(this._item[eve[1]]&&this._item[eve[1]]._com.data[eve[2]])
                    for(let item of eve[3])
                       this._item[eve[1]]._com.data[eve[2]*1][item[0]] = item[1]
                  break
           ...
           case "#pushAct":
               if(this._item[eve[1]]){
                   if(this._item[eve[1]] instanceof LIM.SCENE.Shape){
                       this._item[eve[1]]._com[eve[2]]={
                           index:0,bit:"",cover:0,adso:7,w:0,h:0,
                           x:0,y:0,rota:0,alpha:0,filter:[],
                           clip:[0,0,0,0,0,0,0,0],
                           tone:[0,0,0,0],blend:[0,0,0,0]
                       }
                   }
               }
               break
