...
((_)=>{
    ...
    _.Vessel.prototype.pushAction=function(mode,next){
        ...
        if(this._com.action[fun]){
            ...
            if(item.frame) 
                this._action.push({data:data,time:0,mask:item.mask,frame:item.frame,freq:item.freq,wave:item.wave,com:item.com})
            else {
                this.x=x
                this.y=y
                this.alpha=this._data.alpha
                if(item.com)  this._origin.triggerHandler(item.com)
                if(item.show) this.maskRect(0,0,Graphics.width,Graphics.height)
                else this.maskRect(0,0,0,0)
            }
        }
        else if(!this.isRun(0)) this.setRun(0,true)
    }
    _.Vessel.prototype.maskRect = function(x,y,w,h) {
        this._mask.clear()
    ...
    _.Vessel.prototype.move = function() {
        ...
        if(action.time > 0) {
            let r = LIM.UTILS.waveNum(action.wave, action.frame*action.freq, action.time-1,1)
            this.darwMask(action,r)
    ...
    _.Vessel.prototype.darwMask=function (action,r){
        this._mask.clear()
        this._mask.beginFill()
        if(!action.mask||action.mask.length===0)
            ...
            for(let item of action.mask){
                let r1=item.wave&&item.wave!==action.wave?
                    LIM.UTILS.waveNum(item.wave, action.frame*item.freq, action.time-1,1):r
                ...
                this._mask.drawRect(
                    ...
                    Graphics.width/100*w,Graphics.height/100*h
                )
            }
        }
        this._mask.endFill();
