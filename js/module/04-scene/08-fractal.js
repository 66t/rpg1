        ...
        if (!this.locked) {
            ...
            const promise = new Promise((resolve) => {
                ...
                switch (this._com.mode){
                    case 0:
                        this.recurSierpinski((this.width-min)/2, (this.height-min)/2, min, this._time%this._com.depth);
                        break
                    case 1:
                        this.recurDragonCurve(this.width/1.4, this.height/1.41,min/2.2,-Math.PI / 2,this._time%this._com.depth)
                        break
                    case 2:
                        this.recurFractalTree(this.width/2, this.height-(min/9),min/9,-Math.PI / 2,this._time%this._com.depth)
                        break
                    case 3:
                        this.recurPoincareDisk(this.width/2, this.height/2,min/9,this._time%this._com.depth)
                        break
                    case 4:
                        this.recurElectron(this.width/2, this.height/2,min/3,this._time%this._com.depth)
                        break
                    case 5:
                        this.recurKochCurve(this.width/20, this.height/2.5,this.width/20*19,this.height/2.5,this._time%this._com.depth)
                        break
                }
                resolve();
            });
            promise.then(() => {
                this._time++;
                setTimeout(()=>{ this.locked = false;},170)
