...
Bitmap.prototype._drawTextBody = function(text, tx, ty, maxWidth) {
    if(typeof(this.textColor) != "string") {
        let colors = this.textColor
        let context = this._context;
        let grad;
        let t = Number(colors[0]) / 180 * Math.PI;
        let ww = text.length * this.fontSize;
        let wh = this.fontSize;
        let p1x = tx;
        let p1y = ty;
        let p2x = tx;
        let p2y = ty + wh / 2;
...
WebAudio.prototype._readOgg = function(array) {
    ...
    while (index < array.length) {
        if (this._readFourCharacters(array, index) === 'OggS') {
            ...
            var numcutImagements = array[index++];
            var cutImagements = [];
            for (var i = 0; i < numcutImagements; i++) {
                cutImagements.push(array[index++]);
            }
            for (i = 0; i < numcutImagements; i++) {
                if (this._readFourCharacters(array, index + 1) === 'vorb') {
                    ...
                    if (headerType === 1) {
                        ...
                        this._readMetaData(array, index, cutImagements[i]);
                    }
                    vorbisHeaderFound = true;
                }
                index += cutImagements[i];
            }
